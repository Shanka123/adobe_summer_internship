...
class PGLearner:
	def __init__(self, pa):
		...
		self.input_height = pa.state_len
		self.input_width = pa.state_width
		self.output_height = pa.num_actions
		...
		print ('network_input_height=', pa.state_len)
		print ('network_input_width=', pa.state_width)
		print ('network_output_dim=', pa.num_actions)

		# image representation
		self.l_out = \
			build_pg_network(pa.state_len, pa.state_width, pa.num_actions)
...
def build_pg_network1(input_height, input_width, output_length):
	l_in = lasagne.layers.InputLayer(
		shape=(None, 1, input_height, input_width),
	)
