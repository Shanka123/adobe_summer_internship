...
from heuristic_agent import Heuristic_Agents
...
def launch(pa, pg_resume=None, render=False, repre='image', end='no_new_job'):
  ...
  #     print "MeanEntropy \t %s" % (np.mean(all_entropy))
	...
	#    slow_down_lr_curve.append(np.mean(all_slowdown))
		...
		#    all_test_rews.append(str(test_rew))
		 #   f = open('/tmp/rewards_instant_min_machines_used_overshoot_5000_traj_100_', 'w')
		 #   f.write('\n'.join(all_test_rews))
		 #   f.close()
	 #       pa.unseen = True
...
def test(it,pa ,pg_resume,workloads,episode_max_length=200):
	...
	for ex in range(pa.num_test_ex):
			...
			for k, machine in enumerate(new_env.machines):
				if len(machine.running_tasks) > 0:
					if machine.cpus_left >= 0:
						util+=str(machine.total_cpus - machine.cpus_left) +','
					else:
		...
		logline += str(str(counter-1)+'|'+str(utils) + str(finished_episode_len)) + '\n' + str(sum(new_rews)) + '\n' + str(sum(suffer))  +'\n'
